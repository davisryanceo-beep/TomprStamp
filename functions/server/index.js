...
app.post("/api/modifiers", authenticateToken, async (req, res) => {
  try {
    ...
    const { error } = await db.from("modifiergroups").insert(modifierGroup);
    if (error) throw error;
...
app.put("/api/modifiers/:id", authenticateToken, verifyOwnership('modifiergroups'), async (req, res) => {
  try {
    ...
    const updates = req.body;
    delete updates.id;
    const { error } = await db.from("modifiergroups").update(updates).eq("id", id);
    if (error) throw error;
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.post("/api/public/customers/login", async (req, res) => {
  ...
  try {
    ...
    // In this system, we are using plain text passwords for customers for now
